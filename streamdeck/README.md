...
- NodeCG の socket.io に接続し、`message` で `startTimer` / `stopTimer` /
  `completeRunner` / `resumeRunner` などのメッセージを送信します。
  拡張機能（`src/extension/timekeeping.ts`）側の改修は不要です。
- `timer` / `current-run` レプリカントを購読（ReplicantObserver）し、走者名・
  状態・色をキーに表示します。取りこぼし対策として「再同期」アクションも用意。
...
| タイマー開始                 | `startTimer`                        | —                        |
| タイマー停止                 | `stopTimer`                         | —                        |
| 走者 完走/リタイア           | `completeRunner {index, forfeit}`   | 走者 index・リタイア扱い |
